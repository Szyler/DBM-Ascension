...
DBM = {
	Revision = ("$Revision: 6001 $"):sub(12, -3),
	Version = "6.01",
	DisplayVersion = "6.01", -- the string that is shown as version
	ReleaseRevision = 6001 -- the revision of the latest stable version that is available (for /dbm ver2)
}
...
if LibStub("LibSharedMedia-3.0", true) then
	...
	LSM:Register("font",  "PT Sans Narrow", standardFont, LSM.LOCALE_BIT_western)
	LSM:Register("sound", "Beware ENG", "Interface\\AddOns\\DBM-Core\\sounds\\beware.ogg")
	LSM:Register("sound", "Beware ENG",	"Sound\\Creature\\AlgalonTheObserver\\UR_Algalon_BHole01.wav")
...
do
	...
	function DBM:StartCombat(mod, delay, event, synced, syncedStartHp, syncedEvent)
		...
		if not checkEntry(inCombat, mod) then
			...
			if mod.combatInfo.noCombatInVehicle and UnitInVehicle("player") then 
				return
			end
			if event then
